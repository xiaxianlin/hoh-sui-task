...
export const handleEscrowObjects = async (events: SuiEvent[], type: string) => {
  ...
  for (const event of events) {
    ...
    updates[data.escrow_id].cancelled = event.type.endsWith("::EscrowCancelled");
    updates[data.escrow_id].swapped = event.type.endsWith("::EscrowSwapped");
  ...
  console.log("handleEscrowObjects", updates);
